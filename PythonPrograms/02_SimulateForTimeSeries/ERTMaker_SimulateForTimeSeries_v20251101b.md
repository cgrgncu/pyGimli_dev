# SimulateForTimeSeries(ERTMaker_SimulateForTimeSeries_v20251101b.py)
+ 作者: HsiupoYeh
+ 更新日期: 2026-01-13

### 版本
+ ERTMaker_SimulateForTimeSeries_v20251101b.py
...
\Lite_PyGimli_Env\ERTMaker_SimulateForTimeSeries_v20251101b.py
\Lite_PyGimli_Env\RUN_ERTMaker_SimulateForTimeSeries_v20251101b.bat
```
...
```
+ 呼叫方式: 運行「RUN_ERTMaker_SimulateForTimeSeries_v20251101b.bat」檔案，檔案內容:
```batch
.\PythonEnv\Python.exe ERTMaker_SimulateForTimeSeries_v20251101b.py
PAUSE
```

### 主程式(ERTMaker_SimulateForTimeSeries_v20251101b.py)
+ 設定檔(CreateMeshSettings.json):
```json
{
"SimulateForTimeSeriesSettings_Version":"v20251031b",
...
"InputFile01_MeshVTK_FileName":"Input_ERTMaker_SimulateForTimeSeries/Demo_SyntheticModel.vtk",
"InputFile02_MeshBCMarkersJSON_FileName_Readme":"不論Winodws或Linux或MAC作業系統一律使用「/」描述檔案路徑與資料夾路徑。",
"InputFile02_MeshBCMarkersJSON_FileName":"Input_ERTMaker_SimulateForTimeSeries/Demo_SyntheticModelBCMarkers.json",
"InputFile03_OHM_FileName_Readme":"不論Winodws或Linux或MAC作業系統一律使用「/」描述檔案路徑與資料夾路徑。",
"InputFile03_OHM_FileName":"Input_ERTMaker_SimulateForTimeSeries/Demo_SyntheticModel.ohm",
...
"Output_MainFileName":"Demo_SyntheticModel",
...
#--------------------------------------------
#--------------------------------------------
def SavePNGCSVForTargetAB(i_ab_idx, ab, ab_array, ERTMaker_Info, Output_PNG_Enable):
    import json
    import os
    import numpy as np
    import pygimli as pg
    ...
    electric_potential=np.load(temp_output_filename)
    #--------------------------------------------
...
#--------------------------------------------
# 主程式區塊(使用multiprocessing必須放在這裡，避免重複觸發呼叫)
#--------------------------------------------
if __name__ == '__main__': 
    ...
    #--------------------------------------------
    # 版本資訊
    VERSION_STR = 'v20251101b'
    # 提示畫面
    print('************************************************************')
    print(f'* ERTMaker_SimulateForTimeSeries')
    ...
    #--------------------------------------------
    # 依序模擬放電情境(手動控制多執行緒)
    MAX_CONCURRENT = mp.cpu_count() # 設定同時運行的最大程序數
    active_processes = []    
    print(f'\n開始手動管理 {MAX_CONCURRENT} 個并行程序...')
    #--
    # 用迴圈提交工作
    for i_ab_idx, ab in enumerate(ab_array):
        ...
        # 如果活躍程序數量達到上限，則等待一個程序結束
        while len(active_processes) >= MAX_CONCURRENT:
            time.sleep(0.1) # 暫停一小段時間，避免 CPU 空轉            
            # 再次檢查並清理已結束的程序
            for p in active_processes:
                if not p.is_alive():
                    p.join()  # 確保資源回收
                    active_processes.remove(p)
                    break # 只需要清理一個，就可以騰出空間
        # --- 資源控制邏輯結束 ---
        #--
        # 創建並啟動新程序
        temp_Output_PNG_Enable=temp_json_data["Output_PNG_Enable"]
        p = mp.Process( target=SavePNGCSVForTargetAB, args=(i_ab_idx,ab,ab_array,ERTMaker_Info,temp_Output_PNG_Enable) )
        p.start()
        #--
        active_processes.append(p)
        print(f'任務 AB電極對 #{i_ab_idx+1} 已啟動 ({len(active_processes)} 個程序正在運行)')
        #--
    #--
    # 最後，等待所有剩餘程序完成
    for p in active_processes:
