...
### 版本
+ ERTMaker_Inversion2D_v20260419a.py
+ 修改CSV欄位，增加一個lambda Index。
...
\Lite_PyGimli_Env\ERTMaker_Inversion2D_v20260401a\ERTMaker_Inversion2D_v20260419a.py
\Lite_PyGimli_Env\ERTMaker_Inversion2D_v20260401a\RUN_ERTMaker_Inversion2D_v20260419a.bat
```
+ 也可以放一些必要的輸入範例檔案(並非每個都有用到):
```

```
+ 呼叫方式: 運行「RUN_ERTMaker_Inversion2D_v20260419a.bat」檔案，檔案內容:
```batch
..\PythonEnv\Python.exe RUN_ERTMaker_Inversion2D_v20260419a.py
PAUSE
```
### 主程式(ERTMaker_Inversion2D_v20260419a.py)
...
```python
#**************************************************************************
#   Name: ERTMaker_Inversion2D_v20260419a.py
#   Copyright: 
#   Author: HsiupoYeh
#   Version: v20260419a
...
#==============================================================================
#   逆推流程與觀念:
#   1.先設計符合實際情況的模型網格，也就是製作mesh的相關檔案。這個是必要輸入檔案。若沒有此檔案，也可以用參數自動生成(預設生成三角網格)。
...
#   4.整理視電阻率資料，使V/I資料變成視電阻率，或直接取用視電阻率資料。而且只允許正的視電阻率。一般情況下:
...
#   6.2.根據作者作者指出，如果設定的誤差過低(例如 < 1%)，反演算法會試圖去擬合隨機噪訊，導致模型出現不真實的偽影(Artifacts)。官方預設使用「3%」，個人解讀:被視為包含電極位置誤差、儀器精度與環境噪訊的合理綜合估計值。
...
#   7.2.先關心PhiD，他的計算是PhiD=sum((Data_obs_i-Data_cal_i)/error_i)這只是觀念式。實際計算，Data會轉到log域，error也會轉到log域，則error_i=log(1+(Relative_Error_i))這就是為什麼前面要準備Relative_Error_i。
...
#   write_log_append(f"管理器中的網格數量 = {Info_mgr_Mesh_All_CellCount}")
#   Info_mgr_Mesh_Marker1_CellCount = np.sum(mgr.mesh.cellMarkers()==1)
#   write_log_append(f"管理器中的附加區域網格數量(Marker=1) = {Info_mgr_Mesh_Marker1_CellCount}")
#   Info_mgr_Mesh_Marker2_CellCount = np.sum(mgr.mesh.cellMarkers() == 2)
#   write_log_append(f"管理器中的研究區域網格數量(Marker=2) = {Info_mgr_Mesh_Marker2_CellCount}")
#   --
#   管理器初始化逆推參數域後:
#   Info_mgr_predict_H2_Mesh_CellCount = Info_mgr_Mesh_All_CellCount*4
#   write_log_append(f"管理器順推使用的H2網格，預期數量 = {Info_mgr_Mesh_All_CellCount}x4 = {Info_mgr_predict_H2_Mesh_CellCount}")
#   Info_mgr_H2_Mesh_CellCount = mgr.fop.mesh().cellCount()
#   write_log_append(f"管理器順推使用的H2網格，數量 = {Info_mgr_H2_Mesh_CellCount}")
#   --
#   檢視逆推資料與網格健康度
#   Info_Obs_Data_Count = Ohm_data.size()
#   write_log_append(f"輸入的觀測資料數量(Obs_N): {Info_Obs_Data_Count}")
#   Info_FullMesh_Count = mgr.mesh.cellCount()
#   write_log_append(f"全網格數量或稱原始網格數量(FullMesh_N): {Info_FullMesh_Count}")
#   Info_PaddingMesh_Count = sum(mgr.mesh.cellMarkers() == 1)
#   write_log_append(f"附加網格區域網格數量(PaddingMesh_N): {Info_PaddingMesh_Count}")
#   Info_StudyAreaMesh_Count = sum(mgr.mesh.cellMarkers() == 2)
#   write_log_append(f"研究區域網格數量(StudyAreaMesh_N): {Info_StudyAreaMesh_Count}")
#   Info_Obs_Data_Count_to_StudyAreaMesh_Count_ratio = Info_Obs_Data_Count / Info_StudyAreaMesh_Count
#   write_log_append(f"資料與逆推參數比例或稱觀測資料與研究區域網格數量比例(Obs_N/StduyAreaMesh_N): {Info_Obs_Data_Count_to_StudyAreaMesh_Count_ratio}")
#   write_log_append(f"經驗上:「Obs_N/StudyAreaMesh_N < 1.0 」時，資訊不足 (Underdetermined)，可能導致產生斑點形狀的結果。")
#   write_log_append(f"經驗上:「1.0 < Obs_N/StudyAreaMesh_N < 3.0」時，資訊平衡 (Well-posed)，是最有效率的逆推情況，能兼顧地層細節與運算穩定性。 ")
#   write_log_append(f"經驗上:「3.0 < Obs_N/StudyAreaMesh_N」時，資訊非常充足 (Overdetermined)，逆推結果會非常穩定，雜訊會被平均掉，地層邊界較平滑。要注意過量的觀測資料影響運算效率。")
#   write_log_append(f"隱藏的陷阱:即使資訊充足，但其實資料主要影響電極附近，距離遠的網格並不一定受資料影響。")
#   write_log_append(f"經驗上:可以依照資訊不足、資訊平衡、資訊非常充足等不同情況，嘗試合適的lam。例如:資訊非常充足時，可以將lam從預設的20調整至10，降低模型約束(平滑化)的權重。降低lam的同時也請考慮過擬合的問題。")
#   write_log_append(f"過擬合(Over-fitting)警告: 調低 lam 會增加過擬合風險。")
#   write_log_append(f"檢查點 1: 若最終 chi^2 << 1.0，代表模型可能正在擬合雜訊，應調高 lam。")
#   write_log_append(f"檢查點 2: 若電極附近出現不自然的「點狀高/低阻異常」，代表 lam 過低。")
#   --
#   設定逆推參數(終止疊代任務相關)
#   REF: inversion.h
#   Default_maxIter = 20 #預設值20，開放使用者修改，視為必填項目。
#   Default_dPhi = 2 #預設值2，單位[%]，禁止使用者修改。[終止疊代任務門檻]：定義目標函數最小化的「相對改善率」下限。當疊代效益 < 1% 時，視為模型已達穩定狀態，提前終止逆推，防止對噪聲過度擬合。
#   逆推參數(正則化相關)
...
#   write_log_append(f"目前迭代次數: {Inversion_Iteration_count}")
#   Info_Inversion_dataTrans = str(mgr.inv.dataTrans)
#   write_log_append(f"逆推運算資料轉換方式: {Info_Inversion_dataTrans}")
#   Info_Inversion_modelTrans = str(mgr.inv.modelTrans).replace("\n", " -> ")
#   write_log_append(f"逆推運算模型轉換方式: {Info_Inversion_modelTrans}")
#--
#   Info_LastInversion_StartModelMesh_Value = mgr.inv.startModel
#   #write_log_append(f"起始模型網格數值內容: {Info_LastInversion_StartModelMesh_Value}")
#   Info_LastInversion_StartModelMesh_CellCount = len(Info_LastInversion_StartModelMesh_Value)
#   write_log_append(f"起始模型網格數量 = {Info_LastInversion_StartModelMesh_CellCount}")
...
#   write_log_append(f"起始模型網格數值(Min/Max) = {Info_LastInversion_StartModelMesh_Value_Min:.2f}/{Info_LastInversion_StartModelMesh_Value_Max:.2f}")
#--
...
#   #write_log_append(f"最後一次迭代的順推H2網格數值內容: {Info_LastInversion_ForwordH2ModelMesh_Value}")
#   Info_LastInversion_ForwordH2ModelMesh_CellCount = len(Info_LastInversion_ForwordH2ModelMesh_Value)
#   write_log_append(f"最後一次迭代的順推H2網格數值數量 = {Info_LastInversion_ForwordH2ModelMesh_CellCount}")
...
#   write_log_append(f"最後一次迭代的順推H2網格數值(Min/Max) = {Info_LastInversion_ForwordH2ModelMesh_Value_Min:.2f}/{Info_LastInversion_ForwordH2ModelMesh_Value_Max:.2f}")
#   Info_LastInversion_InversionModelMesh_Value = mgr.inv.modelHistory[Inversion_Iteration_count]
#   #write_log_append(f"最後一次迭代的逆推網格數值內容: {Info_LastInversion_InversionModelMesh_Value}")
#   Info_LastInversion_InversionModelMesh_CellCount = len(Info_LastInversion_InversionModelMesh_Value)
#   write_log_append(f"最後一次迭代的逆推網格數量 = {Info_LastInversion_InversionModelMesh_CellCount}")
...
#   write_log_append(f"最後一次迭代的逆推網格數值(Min/Max) = {Info_LastInversion_InversionModelMesh_Value_Min:.2f}/{Info_LastInversion_InversionModelMesh_Value_Max:.2f}")
#   Info_LastInversion_Obs_dataVals = mgr.inv.dataVals
#   #write_log_append(f"最後一次迭代的觀測資料內容: {Info_LastInversion_Obs_dataVals}")
#   Info_LastInversion_Obs_dataVals_Count = len(Info_LastInversion_Obs_dataVals)
#   write_log_append(f"最後一次迭代的觀測資料數量 = {Info_LastInversion_Obs_dataVals_Count}")
...
#   write_log_append(f"最後一次迭代的觀測資料數值(Min/Max) = {Info_LastInversion_Obs_dataVals_Min:.2f}/{Info_LastInversion_Obs_dataVals_Max:.2f}")
#   Info_LastInversion_Obs_errorVals = mgr.inv.errorVals
#   #write_log_append(f"最後一次迭代的觀測資料估計相對誤差內容: {Info_LastInversion_Obs_errorVals}")
#   Info_LastInversion_Obs_errorVals_Count = len(Info_LastInversion_Obs_errorVals)
#   write_log_append(f"最後一次迭代的觀測資料估計相對誤差數量 = {Info_LastInversion_Obs_errorVals_Count}")
...
#   write_log_append(f"最後一次迭代的觀測資料估計相對誤差數值(Min/Max) = {Info_LastInversion_Obs_errorVals_Min:.2f}/{Info_LastInversion_Obs_errorVals_Max:.2f}")
#   Info_LastInversion_response_dataVals = mgr.inv.response
#   #write_log_append(f"最後一次迭代的計算資料內容: {Info_LastInversion_response_dataVals}")
#   Info_LastInversion_response_dataVals_Count = len(Info_LastInversion_response_dataVals)
#   write_log_append(f"最後一次迭代的計算資料數量 = {Info_LastInversion_response_dataVals_Count}")
...
#   write_log_append(f"最後一次迭代的計算資料數值(Min/Max) = {Info_LastInversion_response_dataVals_Min:.2f}/{Info_LastInversion_response_dataVals_Max:.2f}")
#   Info_LastInversion_Lamda = mgr.inv.lam # 這常常取不到
#   write_log_append(f"最後一次迭代的逆推採用Lamda = {Info_LastInversion_Lamda}")
#   Info_LastInversion_RMS = mgr.inv.absrms()
#   #write_log_append(f"最後一次迭代的RMS = {Info_LastInversion_RMS:.2f}")
#   Info_LastInversion_RRMS = mgr.inv.relrms()
#   #write_log_append(f"最後一次迭代的RRMS = {Info_LastInversion_RRMS:.2f}%")
#   write_log_append(f"最後一次迭代的RMS/RRMS = {Info_LastInversion_RMS:.2f}/{Info_LastInversion_RRMS:.2f}%")
#   Info_LastInversion_chi2 = mgr.inv.chi2()
#   write_log_append(f"最後一次迭代的chi2 = {Info_LastInversion_chi2:.2f}")
#   Info_LastInversion_PhiD = mgr.inv.phiData()
#   #write_log_append(f"最後一次迭代的PhiD = {Info_LastInversion_PhiD:.2f}")
#   Info_LastInversion_PhiM = mgr.inv.phiModel()
#   #write_log_append(f"最後一次迭代的PhiM = {Info_LastInversion_PhiM:.2f}")
#   Info_LastInversion_Phi = mgr.inv.phi()
#   write_log_append(f"最後一次迭代的Phi = PhiD+(lam*PhiM) =: {Info_LastInversion_PhiD:.2f}+({Info_LastInversion_Lamda}*{Info_LastInversion_PhiM:.2f}) = {Info_LastInversion_Phi:.2f}")
#--
# log中的資訊解析:
#==============================================================================
...
#    #    write_log_append(np.min(C), np.max(C))
#
...
#
#        #        write_log_append("min-max nnn ", min(nnn), max(nnn))
...
#
##    else:
##        write_log_append('taking the values directly')
...
def init_log():
    print("--- 工作任務啟動 ---")
    with open("ERTMaker_Inversion2D.log", "w", encoding="utf-8") as f:
        f.write("--- 工作任務啟動 ---\n")
def write_log_append(message):
    print(message)
    with open("ERTMaker_Inversion2D.log", "a", encoding="utf-8") as f:
        f.write(str(message) + "\n")
#--------------------------------------------
# 取得運行開始時間
...
#--------------------------------------------
# 版本資訊
VERSION_STR = 'v20260419a'
# 提示畫面
write_log_append('************************************************************')
write_log_append(f'* ERTMaker_Inversion2D_{VERSION_STR}')
write_log_append('* Author: HsiupoYeh')
write_log_append(f'* Version: {VERSION_STR}')
write_log_append('************************************************************')
# 軟體說明
ERTMaker_Info = f'ERTMaker v1.0(Author: HsiupoYeh) Powered by PyGIMLi v{pg.__version__}'
write_log_append(ERTMaker_Info)
#--
write_log_append(f"程式開始運行...時間: {program_start_time.strftime('%Y-%m-%d %H:%M:%S')}")
write_log_append('--')
#--------------------------------------------
# 指定檔案名稱
...
# 讀取檔案
# 開啟並讀取 JSON 檔案
write_log_append("讀取 .json 檔...")
with open(json_file_name, 'r', encoding='utf-8') as json_file:
    temp_json_data = json.load(json_file)
write_log_append("讀取 .json 檔...完成!")
#--------------------------------------------
# 複製JSON檔案到輸出資料夾
Output_JSON_FileName = os.path.join(temp_json_data['OutputMainFolderPath'],'Inversion2D.json')
if os.path.dirname(Output_JSON_FileName): # 防止空路徑報錯
    os.makedirs(os.path.dirname(Output_JSON_FileName), exist_ok=True)
# 複製檔案
shutil.copy(json_file_name, Output_JSON_FileName)
#--------------------------------------------
# 讀取資料 Ohm檔案
#--
write_log_append("載入Ohm檔案...")
Input_Ohm_FileName = temp_json_data["InputFile01_Ohm_FileName"]
write_log_append(f"目標檔案: {Input_Ohm_FileName}")
#這個棄用Ohm_data = ert.load(Input_Ohm_FileName) # 這個方式會把不合理的資料刪除，若希望保留請使用pg.DataContainerERT(Input_Ohm_FileName,removeInvalid=False)
Ohm_data = pg.DataContainerERT(Input_Ohm_FileName,removeInvalid=False)
if os.path.exists('invalid.data'):
    os.remove('invalid.data')
write_log_append("載入Ohm檔案...完成!")
#--
write_log_append(f"轉換為逆推專用2D座標系統(XY座標)...")
# 如果滿足2D情況，則強迫轉換成X有值，Y有值，Z均為0。
Ohm_data.ensure2D()
if  np.max(np.abs(pg.z(Ohm_data.sensorPositions()))) != 0 :
    write_log_append("Ohm檔案內容不是2D資料，無法進行後續分析，請檢查!")
    exit(1)
write_log_append(f"轉換為逆推專用2D座標系統(XY座標)完成!")
#--
Ohm_data_All_count = Ohm_data.size()
write_log_append(f'Ohm: 感應器數量 = {Ohm_data.sensorCount()}, 資料數量 = {Ohm_data_All_count}')
#--------------------------------------------
write_log_append('--')
#--------------------------------------------
# 準備逆推工具內部要使用的網格
...
if temp_json_data["Mesh_Setting01_AutoMesh_Enable"] == 'Yes' :
    ...
    write_log_append(f"網格節點數量 (Mesh Node Count): {mesh.nodeCount()}")
    write_log_append(f"網格單元數量 (Mesh Cell Count): {mesh.cellCount()}")
    write_log_append(f"附加區域網格單元(Marker=1)的單元數量(Mesh Cell Marker=1 Count): {np.sum(mesh.cellMarkers() == 1)}")
    write_log_append(f"研究區域網格單元(Marker=2)的單元數量(Mesh Cell Marker=2 Count): {np.sum(mesh.cellMarkers() == 2)}")
    write_log_append(f"網格邊界數量 (Mesh Boundary Count): {mesh.boundaryCount()}")
    #--------------------------------------------
    # 填入預設電阻率
    ...
    mesh = pg.Mesh(2)
    # 使用計算出的節點座標創建網格
    write_log_append("讀取 .vtk 檔...")
    mesh.load(temp_json_data["InputFile02_MeshVTK_FileName"])
    write_log_append("讀取 .vtk 檔...完成!")
    # 確保正確性，重新計算log10的電阻率
    mesh['Resistivity_(log10)'] = np.log10(mesh['Resistivity'])
    #--------------------------------------------
    # 讀取並設定 MeshBCMarkers
    write_log_append("讀取 MeshBCMarkersJSON 檔...")
    # 開啟並讀取 JSON 檔案
    ...
    write_log_append("讀取 MeshBCMarkersJSON 檔...完成!")
    #--
    # 取出最後一直欄(marker)
...
try:
    ...
    write_log_append("檢查通過：所有電極均成功匹配網格單元。")
except RuntimeError as e:
    # 當電極在網格之外時，會捕捉到 RuntimeError
    write_log_append("檢查失敗：偵測到電極與網格不匹配！(電極在網格之外嗎?)")
    write_log_append(f"錯誤訊息: {e}")
    exit(1)
write_log_append("檢查網格與電極是否匹配...完成!")
#--------------------------------------------
#==============================================================================
...
if PNG_colorbar_ticker_ModifyLog10ToLinear_Enable:
    ...
    cbar.ax.xaxis.get_major_formatter().set_useOffset(False) #禁用「偏移量 (Offset)」模式
    cbar.ax.xaxis.get_offset_text().set_visible(False)  # 避免顯示 1eX
#--
# 填入版本資訊
...
#--
#write_log_append(f'儲存 {PNG_Profile_Name} 圖片...')
...
plt.close()
#write_log_append(f'儲存 {PNG_Profile_Name} 圖片...完成!')
#==============================================================================
#--------------------------------------------
# 輸出VTK
...
# 輸出
Main_FileName = temp_json_data["Output_MainFileName"]
temp_output_filename=os.path.join(temp_json_data['OutputMainFolderPath'],f'{Main_FileName}_01_InputFullMesh.vtk')
mesh.exportVTK(temp_output_filename)
#-------------------------------------------- 
# 整理 MeshBCMarkers
#--    
# 印出 index, x1, y1, x2, y2, marker
#write_log_append("index, x1, y1, x2, y2, marker")
#for i, b in enumerate(mesh.boundaries()):
#    write_log_append(f"{i}, {b.nodes()[0].pos()[0]:.6f}, {b.nodes()[0].pos()[1]:.6f}, {b.nodes()[1].pos()[0]:.6f}, {b.nodes()[1].pos()[1]:.6f}, {b.marker()}")
#--
Main_FileName = temp_json_data["Output_MainFileName"]
temp_output_filename=os.path.join(temp_json_data['OutputMainFolderPath'],f'{Main_FileName}_01_InputFullMeshBCMarkers.json')
# 儲存MeshMarkers為JSON檔案
write_log_append('儲存MeshBCMarkers為JSON檔案...')
...
with open(temp_output_filename, 'w', encoding='utf-8') as f:
    ...
    f.write('}\n')
    #--
write_log_append('儲存MeshBCMarkers為JSON檔案...完成!')
#--------------------------------------------
#==============================================================================
...
#write_log_append(f'儲存 {PNG_Profile_Name} 圖片...完成!')
#==============================================================================
if temp_json_data["Output_Inversion_XMinMax_Enable"] == 'Yes' :
    #==============================================================================
    # Pygimli網格繪圖:  繪製研究區域網格剖面(電阻率、電極位置、網格資訊)
    #--
    # 可填入的資訊
    ...
    PNG_Width = 1280
    PNG_Height = 720    
    Output_PNG_FileName = os.path.join(temp_json_data['OutputMainFolderPath'],f'{Main_FileName}_01_StudyAreaMesh2.png')
    #--
    # 建立畫布
    fig, ax = plt.subplots(figsize=((PNG_Width / PNG_DPI, PNG_Height / PNG_DPI)))
    # 繪製網格
    ...
    #--
    # 依照參數設定繪製範圍
    ax.set_xlim(temp_json_data['Output_Inversion_XMin'] , temp_json_data['Output_Inversion_XMax'])
    ax.set_ylim(temp_json_data['Output_Inversion_YMin'] , temp_json_data['Output_Inversion_YMax'])
    #--
    #write_log_append(f'儲存 {PNG_Profile_Name} 圖片...')
    #Output_PNG_FileName = 'ABCD.png'
    if os.path.dirname(Output_PNG_FileName): # 防止空路徑報錯
...
#--------------------------------------------
# 重新計算幾何因子與視電阻率並指定誤差
write_log_append("重新計算幾何因子與視電阻率並指定誤差...")
#--------------------------------------------
# K0_Mesh部分 是否要計算
if (temp_json_data["Data_Setting02_GeometricFactorForInversion"] == 'K0_Mesh') or (temp_json_data["OutputFile01_GeometricFactor_K_CrossPlot_PNG_Enable"] == 'Yes'):
    write_log_append('**計算解析解公式的幾何因子!請注意僅適用水平地層!')
    # 計算解析解 (假設水平地面)
    K0_Mesh = ert.createGeometricFactors(Ohm_data, numerical=False)
#--------------------------------------------
# K_Mesh部分 是否要計算
if temp_json_data["Data_Setting02_GeometricFactorForInversion"] == 'K_Mesh' : # 計算網格綁定的幾何因子K_Mesh
    write_log_append('**計算網格綁定的幾何因子K_Mesh!(有考慮地形)')
    ...
    # 建一個暫時的逆推管理
    write_log_append("**建立暫時的ERTManager物件...")
    ...
    write_log_append("**建立暫時的ERTManager物件...完成")
    #--
    write_log_append("**使用暫時的管理器運行逆推(maxIter=0)...")
    temp_mgr.invert(mesh=mesh, maxIter=0, startModel=1.0, verbose=False)
    write_log_append("**使用暫時的管理器運行逆推maxIter=0...完成!")
    # 計算正確的幾何因子(K_Mesh暗示這等效於ert.createGeometricFactors(Ohm_data, mesh=mesh, numerical=True))
    K_Mesh = 1/temp_mgr.inv.response  
elif temp_json_data["Data_Setting02_GeometricFactorForInversion"] == 'K0_Mesh' :   # 使用前面算好的解析解
    K_Mesh = K0_Mesh.copy()
else:
    K0_Mesh = ert.createGeometricFactors(Ohm_data, numerical=False)
    K_Mesh = K0_Mesh.copy()
#--------------------------------------------
# K_Ideal部分 是否要計算
if (temp_json_data["Data_Setting00_RecalculateDataMode"] == 'r') and (temp_json_data["Data_Setting03_GeometricFactorForRecalculateData"] == 'Ideal_Mesh') : # 計算理想網格綁定的幾何因子K_Ideal。要讀取參數來建立理想網格。
    #--
    write_log_append("***從參數建立的理想網格計算理想幾何因子...")
    #--------------------------------------------
    K_Ideal_Mesh=pg.meshtools.createParaMesh(
        Ohm_data,quality=temp_json_data["Ideal_Mesh_Setting01_AutoMesh_quality"],
        paraDepth=temp_json_data["Ideal_Mesh_Setting02_AutoMesh_paraDepth"],
        boundary=temp_json_data["Ideal_Mesh_Setting03_AutoMesh_boundary"],
        paraMaxCellSize=temp_json_data["Ideal_Mesh_Setting04_AutoMesh_paraMaxCellSize"],
        addNodes=temp_json_data["Ideal_Mesh_Setting05_AutoMesh_addNodes"],
        paraDX=temp_json_data["Ideal_Mesh_Setting06_AutoMesh_paraDX"])
    write_log_append(f"****網格節點數量 (K_Ideal_Mesh Node Count): {K_Ideal_Mesh.nodeCount()}")
    write_log_append(f"****網格單元數量 (K_Ideal_Mesh Cell Count): {K_Ideal_Mesh.cellCount()}")
    write_log_append(f"****附加區域網格單元(Marker=1)的單元數量(K_Ideal_Mesh Cell Marker=1 Count): {np.sum(K_Ideal_Mesh.cellMarkers() == 1)}")
    write_log_append(f"****研究區域網格單元(Marker=2)的單元數量(K_Ideal_Mesh Cell Marker=2 Count): {np.sum(K_Ideal_Mesh.cellMarkers() == 2)}")
    write_log_append(f"****網格邊界數量 (K_Ideal_Mesh Boundary Count): {K_Ideal_Mesh.boundaryCount()}")
    #--------------------------------------------
    # 填入預設電阻率
    K_Ideal_Mesh['Resistivity'] = np.ones(K_Ideal_Mesh.cellCount())
    K_Ideal_Mesh['Resistivity_(log10)'] = np.log10(K_Ideal_Mesh['Resistivity'])
    #--------------------------------------------
    # 使用mgr來計算幾何因子
    temp_Ohm_data=Ohm_data.copy()
    # 先填暫時的幾何因子(強制設為1.0)
    ...
    #--
    write_log_append("****使用暫時的管理器運行逆推(maxIter=0)...")
    temp_mgr.invert(mesh=K_Ideal_Mesh, maxIter=0, startModel=1.0, verbose=False)
    write_log_append("****使用暫時的管理器運行逆推maxIter=0...完成!")
    # 計算正確的幾何因子
    K_Ideal = 1/temp_mgr.inv.response 
    temp_Ohm_data['k'] = 1/temp_mgr.inv.response
    temp_Ohm_data.save(os.path.join(temp_json_data['OutputMainFolderPath'],f'{Main_FileName}_K_Ideal.dat'),'a b m n r i rhoa k err')
    #--
elif temp_json_data["Data_Setting03_GeometricFactorForRecalculateData"] == 'K_Mesh' :   # 使用前面算好的數值解K_Mesh
    K_Ideal = K_Mesh.copy()
elif temp_json_data["Data_Setting03_GeometricFactorForRecalculateData"] == 'K0_Mesh' :   # 使用前面算好的解析解 
    K_Ideal = K0_Mesh.copy()
else:
    K_Ideal = K_Mesh.copy()
#--------------------------------------------  
if temp_json_data["Data_Setting00_RecalculateDataMode"] == 'rhoa' and Ohm_data.haveData('rhoa') :
    write_log_append("採用檔案中的「rhoa」欄位!")
    # 選擇逆推用的幾何因子
    write_log_append("綁定目前網格的幾何因子「K_Mesh」!")
    Ohm_data['k'] = K_Mesh.copy()
    # 計算正確的V/I(重新計算)
    write_log_append("重新計算正確的V/I!")
    ...
    if not Ohm_data.haveData('i'):
        write_log_append("自動產生預設電流值(1安培)!")
        Ohm_data['i'] = (Ohm_data['r']*0)+1.0
else:
    write_log_append("採用檔案中的「r」欄位!")
    # 選擇逆推用的幾何因子
    write_log_append("綁定目前網格的幾何因子「K_Mesh」!")
    Ohm_data['k'] = K_Mesh.copy()
    # 計算正確的視電阻率(重新計算)
    write_log_append("重新計算正確的視電阻率!")
    Ohm_data['K_Ideal'] = K_Ideal.copy()
    Ohm_data["rhoa"] = Ohm_data['r'] * Ohm_data['K_Ideal']
    # 提供資料估計相對誤差
    Ohm_data['err'] = ert.estimateError(Ohm_data, relativeError=Data_Settings_Relative_Error_Percentage/100)  # Estimate relative error
    # 確保有電流欄位
    if not Ohm_data.haveData('i'):
        write_log_append("自動產生預設電流值(1安培)!")
        Ohm_data['i'] = (Ohm_data['r']*0)+1.0
write_log_append("重新計算幾何因子與視電阻率並指定誤差...完成!")
#--------------------------------------------
write_log_append("--")
if temp_json_data["OutputFile01_GeometricFactor_K_CrossPlot_PNG_Enable"] == 'Yes' :
    ...
    #--
    # 繪製資料點
    K0_Mesh_positive_positive = K0_Mesh[(K0_Mesh > 0) & (K_Ideal > 0)]
    K_Ideal_positive_positive = K_Ideal[(K0_Mesh > 0) & (K_Ideal > 0)]
    axes[0].scatter(K0_Mesh_positive_positive, K_Ideal_positive_positive, alpha=0.5, s=5, c='blue',label=f'1+2+({len(K0_Mesh_positive_positive)})')
    K0_Mesh_positive_negative = K0_Mesh[(K0_Mesh > 0) & (K_Ideal < 0)]
    K_Ideal_positive_negative = K_Ideal[(K0_Mesh > 0) & (K_Ideal < 0)]
    axes[0].scatter(K0_Mesh_positive_negative, K_Ideal_positive_negative, alpha=0.5, s=5, c='blue',label=f'1+2-({len(K0_Mesh_positive_negative)})')
    K0_Mesh_negative_negative = K0_Mesh[(K0_Mesh < 0) & (K_Ideal < 0)]
    K_Ideal_negative_negative = K_Ideal[(K0_Mesh < 0) & (K_Ideal < 0)]
    axes[0].scatter(K0_Mesh_negative_negative, K_Ideal_negative_negative, alpha=0.5, s=5, c='blue',label=f'1-2-({len(K0_Mesh_negative_negative)})')
    K0_Mesh_negative_positive = K0_Mesh[(K0_Mesh < 0) & (K_Ideal > 0)]
    K_Ideal_negative_positive = K_Ideal[(K0_Mesh < 0) & (K_Ideal > 0)]
    axes[0].scatter(K0_Mesh_negative_positive, K_Ideal_negative_positive, alpha=0.5, s=5, c='blue',label=f'1-2+({len(K0_Mesh_negative_positive)})')
    # 設定等比例
    ...
    #--
    # 調整繪圖參數
    axes[0].set_xlabel(f'1: Half-space Analytical Solution K0_Mesh (m)')
    axes[0].set_ylabel(f'2: Ideal Numerical Solution K_Ideal (m)')
    axes[0].set_title(f'#1 Geometric Factor K Cross Plot(Linear)\nData Count = {len(K0_Mesh_positive_positive)+len(K0_Mesh_positive_negative)+len(K0_Mesh_negative_negative)+len(K0_Mesh_negative_positive)}')
    axes[0].legend(loc='upper left')
    #--
    # 子圖2
    #--
    # 繪製資料點
    K0_Mesh_positive_positive = K0_Mesh[(K0_Mesh > 0) & (K_Mesh > 0)]
    K_Mesh_positive_positive = K_Mesh[(K0_Mesh > 0) & (K_Mesh > 0)]
    axes[1].scatter(K0_Mesh_positive_positive, K_Mesh_positive_positive, alpha=0.5, s=5, c='blue',label=f'1+2+({len(K0_Mesh_positive_positive)})')
    K0_Mesh_positive_negative = K0_Mesh[(K0_Mesh > 0) & (K_Mesh < 0)]
    K_Mesh_positive_negative = K_Mesh[(K0_Mesh > 0) & (K_Mesh < 0)]
    axes[1].scatter(K0_Mesh_positive_negative, K_Mesh_positive_negative, alpha=0.5, s=5, c='blue',label=f'1+2-({len(K0_Mesh_positive_negative)})')
    K0_Mesh_negative_negative = K0_Mesh[(K0_Mesh < 0) & (K_Mesh < 0)]
    K_Mesh_negative_negative = K_Mesh[(K0_Mesh < 0) & (K_Mesh < 0)]
    axes[1].scatter(K0_Mesh_negative_negative, K_Mesh_negative_negative, alpha=0.5, s=5, c='blue',label=f'1-2-({len(K0_Mesh_negative_negative)})')
    K0_Mesh_negative_positive = K0_Mesh[(K0_Mesh < 0) & (K_Mesh > 0)]
    K_Mesh_negative_positive = K_Mesh[(K0_Mesh < 0) & (K_Mesh > 0)]
    axes[1].scatter(K0_Mesh_negative_positive, K_Mesh_negative_positive, alpha=0.5, s=5, c='blue',label=f'1-2+({len(K0_Mesh_negative_positive)})')
    # 設定等比例
    ...
    #--
    # 調整繪圖參數
    axes[1].set_xlabel(f'1: Half-space Analytical Solution K0_Mesh (m)')
    axes[1].set_ylabel(f'2: Custom-Mesh Numerical Solution K_Mesh (m)')
    ...
    #--
    # 繪製資料點
    K_Ideal_positive_positive = K_Ideal[(K_Ideal > 0) & (K_Mesh > 0)]
    K_Mesh_positive_positive = K_Mesh[(K_Ideal > 0) & (K_Mesh > 0)]
    axes[2].scatter(K_Ideal_positive_positive, K_Mesh_positive_positive, alpha=0.5, s=5, c='blue',label=f'1+2+({len(K_Ideal_positive_positive)})')
    K_Ideal_positive_negative = K_Ideal[(K_Ideal > 0) & (K_Mesh < 0)]
    K_Mesh_positive_negative = K_Mesh[(K_Ideal > 0) & (K_Mesh < 0)]
    axes[2].scatter(K_Ideal_positive_negative, K_Mesh_positive_negative, alpha=0.5, s=5, c='blue',label=f'1+2-({len(K_Ideal_positive_negative)})')
    K_Ideal_negative_negative = K_Ideal[(K_Ideal < 0) & (K_Mesh < 0)]
    K_Mesh_negative_negative = K_Mesh[(K_Ideal < 0) & (K_Mesh < 0)]
    axes[2].scatter(K_Ideal_negative_negative, K_Mesh_negative_negative, alpha=0.5, s=5, c='blue',label=f'1-2-({len(K_Ideal_negative_negative)})')
    K_Ideal_negative_positive = K_Ideal[(K_Ideal < 0) & (K_Mesh > 0)]
    K_Mesh_negative_positive = K_Mesh[(K_Ideal < 0) & (K_Mesh > 0)]
    axes[2].scatter(K_Ideal_negative_positive, K_Mesh_negative_positive, alpha=0.5, s=5, c='blue',label=f'1-2+({len(K_Ideal_negative_positive)})')
    # 設定等比例
    ...
    #--
    # 調整繪圖參數
    axes[2].set_xlabel(f'1: Ideal Numerical Solution K_Ideal (m)')
    axes[2].set_ylabel(f'2: Custom-Mesh Numerical Solution K_Mesh (m)')
    axes[2].set_title(f'#3 Geometric Factor K Cross Plot(Linear)\nData Count = {len(K_Ideal_positive_positive)+len(K_Ideal_positive_negative)+len(K_Ideal_negative_negative)+len(K_Ideal_negative_positive)}')
    axes[2].legend(loc='upper left')
    #--
    # 填入總標題
    #if temp_json_data["Data_Setting02_GeometricFactorByMesh_Enable"] == 'Yes' :  
    #    if temp_json_data["IdealGeometricFactorByFile_Enable"] == 'Yes' :
    #        fig.suptitle(f'{Main_FileName}\nGeometric Factor K Cross Plot\nInput File: {os.path.basename(Input_Ohm_FileName)}\nUse Custom-Mesh K Solution.\nIdeal K : Load From File', fontsize=16, fontweight='bold', y =0.9)  
    #    else:
    #        fig.suptitle(f'{Main_FileName}\nGeometric Factor K Cross Plot\nInput File: {os.path.basename(Input_Ohm_FileName)}\nUse Custom-Mesh K Solution.\nIdeal K : Custom Ideal Mesh', fontsize=16, fontweight='bold', y =0.9)  
    #else:
    fig.suptitle(f'Geometric Factor K Cross Plot\n{Main_FileName}: K_Ideal = {temp_json_data["Data_Setting03_GeometricFactorForRecalculateData"]}\nOhm File: {os.path.basename(Input_Ohm_FileName)}', fontsize=16, fontweight='bold', y =0.9)  
    # 填入版本資訊
    fig.text(0.98, 0.01, ERTMaker_Info, ha='right', va='bottom', fontsize=8, color='gray')
    #--
    write_log_append(f'儲存K_CrossPlot圖片...')
    ...
    plt.close()
    write_log_append(f'儲存K_CrossPlot圖片...完成!')
    #--------------------------------------------
    write_log_append("--")
#--------------------------------------------
# 繪圖 V/I All
...
temp_PNG_Width = 1280
temp_PNG_Height = 720
PNG_cMin = np.min(Ohm_data['r']) # None表示自動
PNG_cMax = np.max(Ohm_data['r']) # None表示自動
# 檢查是否所有數值都很接近
if (PNG_cMin - PNG_cMax) < 0.000002:
    # 如果很接近，設定極小的偏移量
    PNG_cMin = PNG_cMin - 0.000001
...
fig.text(0.98, 0.01, ERTMaker_Info, ha='right', va='bottom', fontsize=8, color='gray')
#--
write_log_append(f'儲存 V/I All圖片...')
...
plt.savefig(Output_PNG_FileName, dpi=temp_PNG_DPI)
plt.close()
write_log_append(f'儲存 V/I All圖片...完成!')
#--------------------------------------------
write_log_append('--')
#--------------------------------------------
# 繪圖 Voltage All
...
temp_PNG_Width = 1280
temp_PNG_Height = 720
PNG_cMin = np.min(Ohm_data['r']*Ohm_data['i']) # None表示自動
PNG_cMax = np.max(Ohm_data['r']*Ohm_data['i']) # None表示自動
# 檢查是否所有數值都很接近
if (PNG_cMin - PNG_cMax) < 0.000002:
    # 如果很接近，設定極小的偏移量
    PNG_cMin = PNG_cMin - 0.000001
...
fig.text(0.98, 0.01, ERTMaker_Info, ha='right', va='bottom', fontsize=8, color='gray')
#--
write_log_append(f'儲存 Voltage All圖片...')
...
plt.savefig(Output_PNG_FileName, dpi=temp_PNG_DPI)
plt.close()
write_log_append(f'儲存 Voltage All圖片...完成!')
#--------------------------------------------
write_log_append('--')
#--------------------------------------------
# 繪圖 Current All
...
temp_PNG_Width = 1280
temp_PNG_Height = 720
PNG_cMin = np.min(Ohm_data['i']) # None表示自動
PNG_cMax = np.max(Ohm_data['i']) # None表示自動
# 檢查是否所有數值都很接近
if (PNG_cMin - PNG_cMax) < 0.000002:
    # 如果很接近，設定極小的偏移量
    PNG_cMin = PNG_cMin - 0.000001
...
fig.text(0.98, 0.01, ERTMaker_Info, ha='right', va='bottom', fontsize=8, color='gray')
#--
write_log_append(f'儲存 Current All圖片...')
...
plt.savefig(Output_PNG_FileName, dpi=temp_PNG_DPI)
plt.close()
write_log_append(f'儲存 Current All圖片...完成!')
#--------------------------------------------
write_log_append('--')
#--------------------------------------------
# 繪圖 AppRes All
...
temp_PNG_Width = 1280
temp_PNG_Height = 720
PNG_cMin = np.min(Ohm_data['rhoa']) # None表示自動
PNG_cMax = np.max(Ohm_data['rhoa']) # None表示自動
# 檢查是否所有數值都很接近
if (PNG_cMin - PNG_cMax) < 0.000002:
    # 如果很接近，設定極小的偏移量
    PNG_cMin = PNG_cMin - 0.000001
...
fig.text(0.98, 0.01, ERTMaker_Info, ha='right', va='bottom', fontsize=8, color='gray')
#--
write_log_append(f'儲存 AppRes All圖片...')
...
plt.savefig(Output_PNG_FileName, dpi=temp_PNG_DPI)
plt.close()
write_log_append(f'儲存 AppRes All圖片...完成!')
#--------------------------------------------
write_log_append('--')
#--------------------------------------------
# 繪圖 Error All
...
temp_PNG_Width = 1280
temp_PNG_Height = 720
PNG_cMin = np.min(Ohm_data['err']*100) # None表示自動
PNG_cMax = np.max(Ohm_data['err']*100) # None表示自動
# 檢查是否所有數值都很接近
if (PNG_cMin - PNG_cMax) < 0.000002:
    # 如果很接近，設定極小的偏移量
    PNG_cMin = PNG_cMin - 0.000001
...
fig.text(0.98, 0.01, ERTMaker_Info, ha='right', va='bottom', fontsize=8, color='gray')
#--
write_log_append(f'儲存 Error All圖片...')
...
plt.savefig(Output_PNG_FileName, dpi=temp_PNG_DPI)
plt.close()
write_log_append(f'儲存 Error All圖片...完成!')
#--------------------------------------------
write_log_append('--')
#--------------------------------------------
...
temp_PNG_Width = 1280
temp_PNG_Height = 720
PNG_cMin = np.min(Ohm_data['k']) # None表示自動
PNG_cMax = np.max(Ohm_data['k']) # None表示自動
# 檢查是否所有數值都很接近
if (PNG_cMin - PNG_cMax) < 0.000002:
    # 如果很接近，設定極小的偏移量
    PNG_cMin = PNG_cMin - 0.000001
...
fig.text(0.98, 0.01, ERTMaker_Info, ha='right', va='bottom', fontsize=8, color='gray')
#--
write_log_append(f'儲存 Geometric_Factor All圖片...')
...
plt.savefig(Output_PNG_FileName, dpi=temp_PNG_DPI)
plt.close()
write_log_append(f'儲存 Geometric_Factor All圖片...完成!')
#--------------------------------------------
write_log_append('--')
#--------------------------------------------
# 繪圖 Geometric_Factor MeshEffect = K_Mesh / K_Ideal
Main_FileName = temp_json_data["Output_MainFileName"]
temp_PNG_DPI = 100
temp_PNG_Width = 1280
temp_PNG_Height = 720
Ohm_data['MeshEffect'] = Ohm_data['k'] / K_Ideal *100 #MeshEffect = K_Mesh / K_Ideal
PNG_cMin = np.min(Ohm_data['MeshEffect']) # None表示自動
PNG_cMax = np.max(Ohm_data['MeshEffect']) # None表示自動
# 檢查是否所有數值都很接近
if (PNG_cMin - PNG_cMax) < 0.000002:
    # 如果很接近，設定極小的偏移量
    PNG_cMin = PNG_cMin - 0.000001
...
# 確保是座標軸對象，並加入標題與軸標籤
ax.set_title(f"{os.path.basename(Input_Ohm_FileName)}\nMeshEffect(K_Mesh / K_Ideal) Distribution ({Ohm_data_All_count}) Min/Max={np.min(Ohm_data['MeshEffect']):.6f}/{np.max(Ohm_data['MeshEffect']):.6f}", fontsize=14)
ax.set_xlabel("")
# 不用科學記號的 colorbar 數值
cbar.ax.xaxis.set_major_formatter(plt.matplotlib.ticker.ScalarFormatter(useMathText=False))
cbar.ax.xaxis.get_major_formatter().set_useOffset(False) #禁用「偏移量 (Offset)」模式
...
fig.text(0.98, 0.01, ERTMaker_Info, ha='right', va='bottom', fontsize=8, color='gray')
#--
write_log_append(f'儲存 Geometric_Factor MeshEffect圖片...')
...
plt.savefig(Output_PNG_FileName, dpi=temp_PNG_DPI)
plt.close()
write_log_append(f'儲存 Geometric_Factor MeshEffect圖片...完成!')
#--------------------------------------------
write_log_append('--')
if temp_json_data["Data_Setting12_UseFakeData_Enable"] == 'Yes' :
    write_log_append("使用假的視電阻率資料...")
    Ohm_data['rhoa'] = (Ohm_data['rhoa']*0) + float(temp_json_data["Data_Setting13_UseFakeData_rhoa"]) 
    Ohm_data['r'] = Ohm_data['rhoa']/Ohm_data['k']
    ...
    write_log_append("使用假的視電阻率資料...完成!")
else:
    #--------------------------------------------
    write_log_append("移除負的視電阻率資料...")
    # PyGimli本來就不允許電阻率為0，所以不必特別移除為0的
    write_log_append(f"移除之前的觀測資料數量: {Ohm_data_All_count}")
    Ohm_data.remove(Ohm_data['rhoa'] < 0)
    Ohm_data_Accept_count = Ohm_data.size()
    write_log_append(f"移除之後的觀測資料數量: {Ohm_data_Accept_count}")
    write_log_append("移除負的視電阻率資料...完成!")
    write_log_append('--')
    #--
    write_log_append(f"移除過小電流(<{temp_json_data["Data_Setting03_RemoveLowCurrentData_LowerThan_A"]}[A])的視電阻率資料...")
    write_log_append(f"移除之前的觀測資料數量: {Ohm_data_All_count}")
    Ohm_data.remove(Ohm_data['i'] < temp_json_data["Data_Setting03_RemoveLowCurrentData_LowerThan_A"])
    Ohm_data_Accept_count = Ohm_data.size()
    write_log_append(f"移除之後的觀測資料數量: {Ohm_data_Accept_count}")
    write_log_append(f"移除過小電流(<{temp_json_data["Data_Setting03_RemoveLowCurrentData_LowerThan_A"]}[A])的視電阻率資料...完成!")
    write_log_append('--')
    #--
    write_log_append(f"移除過大電流(>{temp_json_data["Data_Setting04_RemoveHighCurrentData_HigherThan_A"]}[A])的視電阻率資料...")
    write_log_append(f"移除之前的觀測資料數量: {Ohm_data_All_count}")
    Ohm_data.remove(Ohm_data['i'] > temp_json_data["Data_Setting04_RemoveHighCurrentData_HigherThan_A"])
    Ohm_data_Accept_count = Ohm_data.size()
    write_log_append(f"移除之後的觀測資料數量: {Ohm_data_Accept_count}")
    write_log_append(f"移除過大電流(>{temp_json_data["Data_Setting04_RemoveHighCurrentData_HigherThan_A"]}[A])的視電阻率資料...完成!")
    write_log_append('--')
    #--
    write_log_append(f"移除過小電壓(<{temp_json_data["Data_Setting05_RemoveLowVoltageData_LowerThan_V"]}[V])的視電阻率資料...")
    write_log_append(f"移除之前的觀測資料數量: {Ohm_data_All_count}")
    Ohm_data.remove((Ohm_data['r']*Ohm_data['i']) < temp_json_data["Data_Setting05_RemoveLowVoltageData_LowerThan_V"])
    Ohm_data_Accept_count = Ohm_data.size()
    write_log_append(f"移除之後的觀測資料數量: {Ohm_data_Accept_count}")
    write_log_append(f"移除過小電壓(<{temp_json_data["Data_Setting05_RemoveLowVoltageData_LowerThan_V"]}[V])的視電阻率資料...完成!")
    write_log_append('--')
    #--
    write_log_append(f"移除過大電壓(>{temp_json_data["Data_Setting06_RemoveHighVoltageData_HigherThan_V"]}[V])的視電阻率資料...")
    write_log_append(f"移除之前的觀測資料數量: {Ohm_data_All_count}")
    Ohm_data.remove((Ohm_data['r']*Ohm_data['i']) > temp_json_data["Data_Setting06_RemoveHighVoltageData_HigherThan_V"])
    Ohm_data_Accept_count = Ohm_data.size()
    write_log_append(f"移除之後的觀測資料數量: {Ohm_data_Accept_count}")
    write_log_append(f"移除過大電壓(>{temp_json_data["Data_Setting06_RemoveHighVoltageData_HigherThan_V"]}[V])的視電阻率資料...完成!")
    write_log_append('--')
    #--
    write_log_append(f"移除過小視電阻率(<{temp_json_data["Data_Setting07_RemoveLowAppResData_LowerThan_OhmM"]}[Ohm-m])的視電阻率資料...")
    write_log_append(f"移除之前的觀測資料數量: {Ohm_data_All_count}")
    Ohm_data.remove(Ohm_data['rhoa'] < temp_json_data["Data_Setting07_RemoveLowAppResData_LowerThan_OhmM"])
    Ohm_data_Accept_count = Ohm_data.size()
    write_log_append(f"移除之後的觀測資料數量: {Ohm_data_Accept_count}")
    write_log_append(f"移除過小視電阻率(<{temp_json_data["Data_Setting07_RemoveLowAppResData_LowerThan_OhmM"]}[Ohm-m])的視電阻率資料...完成!")
    write_log_append('--')
    #--
    write_log_append(f"移除過大視電阻率(>{temp_json_data["Data_Setting08_RemoveHighAppResData_HigherThan_OhmM"]}[Ohm-m])的視電阻率資料...")
    write_log_append(f"移除之前的觀測資料數量: {Ohm_data_All_count}")
    Ohm_data.remove(Ohm_data['rhoa'] > temp_json_data["Data_Setting08_RemoveHighAppResData_HigherThan_OhmM"])
    Ohm_data_Accept_count = Ohm_data.size()
    write_log_append(f"移除之後的觀測資料數量: {Ohm_data_Accept_count}")
    write_log_append(f"移除過大視電阻率(>{temp_json_data["Data_Setting08_RemoveHighAppResData_HigherThan_OhmM"]}[Ohm-m])的視電阻率資料...完成!")
    write_log_append('--')
    #--
    write_log_append(f"移除過小網格效應(<{temp_json_data["Data_Setting10_RemoveLowMeshEffectData_LowerThan_Percent"]}[%])的視電阻率資料...")
    write_log_append(f"移除之前的觀測資料數量: {Ohm_data_All_count}")
    Ohm_data.remove(Ohm_data['MeshEffect'] < temp_json_data["Data_Setting10_RemoveLowMeshEffectData_LowerThan_Percent"])
    Ohm_data_Accept_count = Ohm_data.size()
    write_log_append(f"移除之後的觀測資料數量: {Ohm_data_Accept_count}")
    write_log_append(f"移除過小網格效應(<{temp_json_data["Data_Setting10_RemoveLowMeshEffectData_LowerThan_Percent"]}[%])的視電阻率資料...完成!")
    write_log_append('--')
    #--
    write_log_append(f"移除過大網格效應(>{temp_json_data["Data_Setting11_RemoveHighMeshEffectData_HigherThan_Percent"]}[%])的視電阻率資料...")
    write_log_append(f"移除之前的觀測資料數量: {Ohm_data_All_count}")
    Ohm_data.remove(Ohm_data['MeshEffect'] > temp_json_data["Data_Setting11_RemoveHighMeshEffectData_HigherThan_Percent"])
    Ohm_data_Accept_count = Ohm_data.size()
    write_log_append(f"移除之後的觀測資料數量: {Ohm_data_Accept_count}")
    write_log_append(f"移除過大網格效應(>{temp_json_data["Data_Setting11_RemoveHighMeshEffectData_HigherThan_Percent"]}[%])的視電阻率資料...完成!")
    write_log_append('--')
    #--
    All_Electrode_Index_1Base = np.arange(Ohm_data.sensorCount())
    All_Electrode_Index_0Base = All_Electrode_Index_1Base-1
    ...
    Good_Electrode_Index_0Base = Good_Electrode_Index_1Base-1
    write_log_append(f"移除不採用(損壞)電極(數量={len(Bad_Electrode_Index_1Base)},清單={Bad_Electrode_Index_1Base})的視電阻率資料...")
    write_log_append(f"移除之前的觀測資料數量: {Ohm_data_All_count}")
    for sensor in ['a', 'b', 'm', 'n']:
        Ohm_data.remove(np.isin(Ohm_data[sensor], Bad_Electrode_Index_1Base))
    Ohm_data_Accept_count = Ohm_data.size()
    write_log_append(f"移除之後的觀測資料數量: {Ohm_data_Accept_count}")
    write_log_append(f"移除不採用(損壞)電極(數量={len(Bad_Electrode_Index_1Base)},清單={Bad_Electrode_Index_1Base})的視電阻率資料...完成!")
    #--------------------------------------------
if Ohm_data_Accept_count == 0:
    write_log_append('--')
    write_log_append(f"錯誤：沒有可用的觀測資料。觀測資料數量 = {Ohm_data_Accept_count}。")
    write_log_append('--')
    exit(1)
write_log_append('--')
#--------------------------------------------
...
fig.text(0.98, 0.01, ERTMaker_Info, ha='right', va='bottom', fontsize=8, color='gray')
#--
write_log_append(f'儲存 V/I Accept圖片...')
...
plt.savefig(Output_PNG_FileName, dpi=temp_PNG_DPI)
plt.close()
write_log_append(f'儲存 V/I Accept圖片...完成!')
#--------------------------------------------
write_log_append('--')
#--------------------------------------------
# 繪圖 Voltage Accept
...
fig.text(0.98, 0.01, ERTMaker_Info, ha='right', va='bottom', fontsize=8, color='gray')
#--
write_log_append(f'儲存 Voltage Accept圖片...')
...
plt.savefig(Output_PNG_FileName, dpi=temp_PNG_DPI)
plt.close()
write_log_append(f'儲存 Voltage Accept圖片...完成!')
#--------------------------------------------
write_log_append('--')
#--------------------------------------------
# 繪圖 Current Accept
...
fig.text(0.98, 0.01, ERTMaker_Info, ha='right', va='bottom', fontsize=8, color='gray')
#--
write_log_append(f'儲存 Current Accept圖片...')
...
plt.savefig(Output_PNG_FileName, dpi=temp_PNG_DPI)
plt.close()
write_log_append(f'儲存 Current Accept圖片...完成!')
#--------------------------------------------
write_log_append('--')
#--------------------------------------------
# 繪圖 AppRes Accept
...
fig.text(0.98, 0.01, ERTMaker_Info, ha='right', va='bottom', fontsize=8, color='gray')
#--
write_log_append(f'儲存 AppRes Accept圖片...')
...
plt.savefig(Output_PNG_FileName, dpi=temp_PNG_DPI)
plt.close()
write_log_append(f'儲存 AppRes Accept圖片...完成!')
#--------------------------------------------
write_log_append('--')
#--------------------------------------------
# 繪圖 Error Accept
...
fig.text(0.98, 0.01, ERTMaker_Info, ha='right', va='bottom', fontsize=8, color='gray')
#--
write_log_append(f'儲存 Error Accept圖片...')
...
plt.savefig(Output_PNG_FileName, dpi=temp_PNG_DPI)
plt.close()
write_log_append(f'儲存 Error Accept圖片...完成!')
#--------------------------------------------
write_log_append('--')
#--------------------------------------------
...
fig.text(0.98, 0.01, ERTMaker_Info, ha='right', va='bottom', fontsize=8, color='gray')
#--
write_log_append(f'儲存 Geometric_Factor Accept圖片...')
...
plt.savefig(Output_PNG_FileName, dpi=temp_PNG_DPI)
plt.close()
write_log_append(f'儲存 Geometric_Factor Accept圖片...完成!')
#--------------------------------------------
write_log_append('--')
#--------------------------------------------
# 繪圖 Geometric_Factor MeshEffect = K_Mesh / K_Ideal Accept
Main_FileName = temp_json_data["Output_MainFileName"]
temp_PNG_DPI = 100
temp_PNG_Width = 1280
temp_PNG_Height = 720
...
#--
write_log_append(f'儲存 AppRes_Histogram 統計圖片...')
...
plt.savefig(Output_PNG_FileName, dpi=temp_PNG_DPI)
plt.close()
write_log_append(f'儲存 AppRes_Histogram 統計圖片...完成!')
#--------------------------------------------
write_log_append("--")
...
def Inversion_Info_Update(i, inv):
    ...
    write_log_append(f"逆推運算模型轉換方式: {Info_Inversion_modelTrans}")
    if mgr.sr == True :
        write_log_append("逆推運算SR技術: 開啟")
        Info_Inversion_SR = 'Yes'
    else :
        write_log_append("逆推運算SR技術: 關閉")
        Info_Inversion_SR = 'No'
    if temp_json_data["Data_Setting02_GeometricFactorForInversion"] == 'K_Mesh' :
        write_log_append("逆推運算幾何因子: 數值模擬計算(去除地形效應)")
        Info_GeometricFactorByMesh = 'Yes'
    else :
        write_log_append("逆推運算幾何因子: 解析解(僅適用水平地層)")
    ...
    write_log_append(f"起始模型網格數值(Min/Max) = {Info_LastInversion_StartModelMesh_Value_Min:.2f}/{Info_LastInversion_StartModelMesh_Value_Max:.2f}")
    #--------------------------------------------    
    write_log_append(f"限制模型網格數值(Min/Max) = {Inversion_limitModelCellMinValue:.2f}/{Inversion_limitModelCellMaxValue:.2f}")
    #--------------------------------------------    
    Info_LastInversion_ForwordH2ModelMesh_Value = mgr.fop.createMappedModel(mgr.inv.model) # 研究區域照填，附加區域延續（prolongated）
    #write_log_append(f"最後一次迭代的順推H2網格數值內容: {Info_LastInversion_ForwordH2ModelMesh_Value}")
    Info_LastInversion_ForwordH2ModelMesh_CellCount = len(Info_LastInversion_ForwordH2ModelMesh_Value)
    write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: 順推H2網格數值數量 = {Info_LastInversion_ForwordH2ModelMesh_CellCount}")
    Info_LastInversion_ForwordH2ModelMesh_Value_Min = np.min(Info_LastInversion_ForwordH2ModelMesh_Value)
    Info_LastInversion_ForwordH2ModelMesh_Value_Max = np.max(Info_LastInversion_ForwordH2ModelMesh_Value)
    write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: 順推H2網格數值(Min/Max) = {Info_LastInversion_ForwordH2ModelMesh_Value_Min:.2f}/{Info_LastInversion_ForwordH2ModelMesh_Value_Max:.2f}")
    #--
    #write_log_append("取出最後一次逆推的H2全網格...")
    LastInversion_H2FullMesh = pg.Mesh(mgr.fop.mesh())
    #write_log_append("取出最後一次逆推的H2全網格...完成!")
    #write_log_append("最後一次逆推的H2全網格填充數值...")
    ...
    #write_log_append("最後一次逆推的H2全網格填充數值...完成!")    
    #--------------------------------------------
    # 輸出VTK
    # 輸出VTK前先把預設該存在的資料填1
    ...
    #write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: 逆推網格數值內容: {Info_LastInversion_InversionModelMesh_Value}")
    Info_LastInversion_InversionModelMesh_CellCount = len(Info_LastInversion_InversionModelMesh_Value)
    write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: 逆推網格數量 = {Info_LastInversion_InversionModelMesh_CellCount}")
    Info_LastInversion_InversionModelMesh_Value_Min = np.min(Info_LastInversion_InversionModelMesh_Value)
    Info_LastInversion_InversionModelMesh_Value_Max = np.max(Info_LastInversion_InversionModelMesh_Value)
    write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: 逆推網格數值(Min/Max) = {Info_LastInversion_InversionModelMesh_Value_Min:.2f}/{Info_LastInversion_InversionModelMesh_Value_Max:.2f}")
    #--
    #write_log_append("取出最後一次逆推的研究區域網格...")
    LastInversion_InversionModelMesh = pg.Mesh(mgr.paraDomain)
    #write_log_append("取出最後一次逆推的研究區域網格...完成!")
    #write_log_append("最後一次逆推的研究區域網格填充數值...")
    ...
    #write_log_append("最後一次逆推的研究區域網格填充數值...完成!")
    #--------------------------------------------
    # 輸出VTK
    # 輸出    
    ...
    #--------------------------------------------
    Info_LastInversion_Obs_dataVals = mgr.inv.dataVals
    #write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: 觀測資料內容: {Info_LastInversion_Obs_dataVals}")
    Info_LastInversion_Obs_dataVals_Count = len(Info_LastInversion_Obs_dataVals)
    write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: 觀測資料數量 = {Info_LastInversion_Obs_dataVals_Count}")
    Info_LastInversion_Obs_dataVals_Min = np.min(Info_LastInversion_Obs_dataVals)
    Info_LastInversion_Obs_dataVals_Max = np.max(Info_LastInversion_Obs_dataVals)
    write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: 觀測資料數值(Min/Max) = {Info_LastInversion_Obs_dataVals_Min:.2f}/{Info_LastInversion_Obs_dataVals_Max:.2f}")
    Info_LastInversion_Obs_errorVals = mgr.inv.errorVals
    #write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: 觀測資料估計相對誤差內容: {Info_LastInversion_Obs_errorVals}")
    Info_LastInversion_Obs_errorVals_Count = len(Info_LastInversion_Obs_errorVals)
    write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: 觀測資料估計相對誤差數量 = {Info_LastInversion_Obs_errorVals_Count}")
    Info_LastInversion_Obs_errorVals_Min = np.min(Info_LastInversion_Obs_errorVals)
    Info_LastInversion_Obs_errorVals_Max = np.max(Info_LastInversion_Obs_errorVals)
    write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: 觀測資料估計相對誤差數值(Min/Max) = {Info_LastInversion_Obs_errorVals_Min:.2f}/{Info_LastInversion_Obs_errorVals_Max:.2f}")
    Info_LastInversion_response_dataVals = mgr.inv.response
    #write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: 計算資料內容: {Info_LastInversion_response_dataVals}")
    Info_LastInversion_response_dataVals_Count = len(Info_LastInversion_response_dataVals)
    write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: 計算資料數量 = {Info_LastInversion_response_dataVals_Count}")
    Info_LastInversion_response_dataVals_Min = np.min(Info_LastInversion_response_dataVals)
    Info_LastInversion_response_dataVals_Max = np.max(Info_LastInversion_response_dataVals)    
    write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: 計算資料數值(Min/Max) = {Info_LastInversion_response_dataVals_Min:.2f}/{Info_LastInversion_response_dataVals_Max:.2f}")
    Info_LastInversion_Lamda = mgr.inv.inv.getLambda()
    mgr.inv.inv.setLambda(Info_LastInversion_Lamda)
    write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: 逆推採用Lamda = {Info_LastInversion_Lamda}")
    Info_LastInversion_RMS = mgr.inv.absrms()
    #write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: RMS = {Info_LastInversion_RMS:.2f}")
    Info_LastInversion_RRMS = mgr.inv.relrms()
    #write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: RRMS = {Info_LastInversion_RRMS:.2f}%")
    write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: RMS/RRMS = {Info_LastInversion_RMS:.2f}/{Info_LastInversion_RRMS:.2f}%")
    Info_LastInversion_chi2 = mgr.inv.chi2()
    write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: chi2 = {Info_LastInversion_chi2:.2f}")
    Info_LastInversion_PhiD = mgr.inv.phiData()
    #write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: PhiD = {Info_LastInversion_PhiD:.2f}")
    Info_LastInversion_PhiM = mgr.inv.phiModel()
    #write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: PhiM = {Info_LastInversion_PhiM:.2f}")
    Info_LastInversion_Phi = mgr.inv.phi()
    write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: Phi = PhiD+(lam*PhiM) =: {Info_LastInversion_PhiD:.2f}+({Info_LastInversion_Lamda}*{Info_LastInversion_PhiM:.2f}) = {Info_LastInversion_Phi:.2f}")
    #--------------------------------------------
    ...
    PNG_Main_Title01 = f"{PNG_Profile_Name}\n Observed vs. Predicted Cross Plot (Linear) " + \
        ...
        rf"$\lambda${Current_lamda_number}={Info_LastInversion_Lamda:.2f}, " + \
    ...
    fig.text(0.98, 0.005, ERTMaker_Info, ha='right', va='bottom', fontsize=8, color='gray')
    #--
    #write_log_append(f'儲存 圖片...')
    if os.path.dirname(Output_PNG_FileName): # 防止空路徑報錯
        os.makedirs(os.path.dirname(Output_PNG_FileName), exist_ok=True)
    plt.tight_layout() 
    plt.savefig(Output_PNG_FileName, dpi=temp_PNG_DPI)
    plt.close()
    #write_log_append(f'儲存 圖片...完成!')
    #--------------------------------------------
    # 相對殘差百分比 統計圖
    ...
    #write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_LastInversion_Relative_Misfit_Percentage}的計算資料內容: {Info_LastInversion_Relative_Misfit_Percentage}")
    Info_LastInversion_Relative_Misfit_Percentage_Count = len(Info_LastInversion_Relative_Misfit_Percentage)
    write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: 相對殘差百分比數量 = {Info_LastInversion_Relative_Misfit_Percentage_Count}")
    ...
    write_log_append(f"Remove_Runs#{Current_Remove_Runs}({Inversion_Remove_Data_Percentage}%),Iteration#{Info_Iteration_count}: 相對殘差百分比數值(Min/Mean/Max) = {Info_LastInversion_Relative_Misfit_Percentage_Min:.2f}%/{Info_LastInversion_Relative_Misfit_Percentage_Mean:.2f}%/{Info_LastInversion_Relative_Misfit_Percentage_Max:.2f}%")
    #--
    # 可填入的資訊
    PNG_Profile_Name = Main_FileName
    ...
    #write_log_append(f"轉換為逆推專用2D座標系統(XY座標)...")
    # 如果滿足2D情況，則強迫轉換成X有值，Y有值，Z均為0。
    LastInversion_Data.ensure2D()
    if  np.max(np.abs(pg.z(LastInversion_Data.sensorPositions()))) != 0 :
        write_log_append("Ohm檔案內容不是2D資料，無法進行後續分析，請檢查!")
        exit(1)
    #write_log_append(f"轉換為逆推專用2D座標系統(XY座標)完成!")
    #--
    # 轉換為標準XZ專用2D座標系統
    #write_log_append(f"轉換為標準2D座標系統(XZ座標)...")
    temp_sensors=LastInversion_Data.sensors()
    pg.core.swapYZ(temp_sensors)
    LastInversion_Data.setSensorPositions(temp_sensors)
    #write_log_append(f"轉換為標準2D座標系統(XZ座標)...完成!")
    #--
    ...
    #write_log_append(f"轉換為逆推專用2D座標系統(XY座標)...")
    # 如果滿足2D情況，則強迫轉換成X有值，Y有值，Z均為0。
    LastInversion_CalData.ensure2D()
    if  np.max(np.abs(pg.z(LastInversion_CalData.sensorPositions()))) != 0 :
        write_log_append("Ohm檔案內容不是2D資料，無法進行後續分析，請檢查!")
        exit(1)
    #write_log_append(f"轉換為逆推專用2D座標系統(XY座標)完成!")
    #--
    # 轉換為標準XZ專用2D座標系統
    #write_log_append(f"轉換為標準2D座標系統(XZ座標)...")
    temp_sensors=LastInversion_CalData.sensors()
    pg.core.swapYZ(temp_sensors)
    LastInversion_CalData.setSensorPositions(temp_sensors)
    #write_log_append(f"轉換為標準2D座標系統(XZ座標)...完成!")
    #--
    ...
    #--------------------------------------------
    #--------------------------------------------
    #write_log_append('儲存 Output_Inversion_Log 檔案...')   
    ...
    with open(Output_Inversion_Log, 'a', encoding='utf-8') as f:
        ...
        f'{Current_lamda_number},{Info_LastInversion_Lamda},{Info_LastInversion_RMS},{Info_LastInversion_RRMS},{Info_LastInversion_chi2},{Info_LastInversion_Phi},{Info_LastInversion_PhiD},{Info_LastInversion_PhiM}\n'    
        f.write(line)
        #--
    #write_log_append('儲存 Output_Inversion_Log 檔案...完成!')
    #--------------------------------------------
    #==============================================================================
    ...
    if temp_json_data["Output_Inversion_XMinMax_Enable"] == 'Yes' :
        #==============================================================================
        # Pygimli網格繪圖:  繪製研究區域網格逆推剖面(電阻率、電極位置、逆推資訊) 無coverage 無網格 自訂colorbar 自訂XY範圍#3a
        #--    
        # 可填入的資訊
        PNG_Profile_Name = Main_FileName
        PNG_NumPy_Electrode_x = LastInversion_Data.sensors().array()[:, 0]
        ...
        PNG_Width = 1280
        PNG_Height = 720    
        Output_PNG_FileName = os.path.join(temp_json_data['OutputMainFolderPath'],f'{Main_FileName}_R{Current_Remove_Runs:02d}_L{Current_lamda_number:02d}_I{Info_Iteration_count:02d}_INV3a.png')
        #--
        # 建立畫布
        fig, ax = plt.subplots(figsize=((PNG_Width / PNG_DPI, PNG_Height / PNG_DPI)))
        # 繪製網格
    ...
    PNG_Width = 1280
    PNG_Height = 720    
    Output_PNG_FileName = os.path.join(temp_json_data['OutputMainFolderPath'],f'{Main_FileName}_R{Current_Remove_Runs:02d}_L{Current_lamda_number:02d}_I{Info_Iteration_count:02d}_INVC2.png')
    #--
    # 建立畫布
    fig, ax = plt.subplots(figsize=((PNG_Width / PNG_DPI, PNG_Height / PNG_DPI)))
    # 繪製網格
    ...
    PNG_Width = 1280
    PNG_Height = 720    
    Output_PNG_FileName = os.path.join(temp_json_data['OutputMainFolderPath'],f'{Main_FileName}_R{Current_Remove_Runs:02d}_L{Current_lamda_number:02d}_I{Info_Iteration_count:02d}_INVC3.png')
    #--
    # 建立畫布
    fig, ax = plt.subplots(figsize=((PNG_Width / PNG_DPI, PNG_Height / PNG_DPI)))
    # 繪製網格
    ...
    if temp_json_data["Output_Inversion_XMinMax_Enable"] == 'Yes' :
        ...
        PNG_Width = 1280
        PNG_Height = 720    
        Output_PNG_FileName = os.path.join(temp_json_data['OutputMainFolderPath'],f'{Main_FileName}_R{Current_Remove_Runs:02d}_L{Current_lamda_number:02d}_I{Info_Iteration_count:02d}_INVC3a.png')
        #--
        # 建立畫布
        fig, ax = plt.subplots(figsize=((PNG_Width / PNG_DPI, PNG_Height / PNG_DPI)))
        # 繪製網格
...
write_log_append(f"遞減Lamda清單 = {Inversion_lam_list}")
Inversion_lam_list_count = len(Inversion_lam_list) 
write_log_append(f"遞減Lamda清單數量 = {Inversion_lam_list_count}")
write_log_append("--")
...
if temp_json_data["Inversion_Setting05_startModel"] > 0 :
    ...
    #--------------------------------------------
    write_log_append(f"全網格數量={mesh.cellCount()},研究區域網格數量={np.sum(mesh.cellMarkers() == 2)}.")
    write_log_append("使用電阻率欄位設定起始模型電阻率...")
    if mesh.haveData("Resistivity"):
        write_log_append(f'設定起始模型電阻率(Min/Max) = {np.min(mesh["Resistivity"][(mesh.cellMarkers() == 2)])}/{np.max(mesh["Resistivity"][(mesh.cellMarkers() == 2)])}')
        Inversion_startModel = mesh["Resistivity"][(mesh.cellMarkers() == 2)]
    else:
        write_log_append('沒有找到電阻率資訊，改用資料的中位數的均質模型。')
        Inversion_startModel = None #預設值「None」，None表示使用資料的中位數。
    write_log_append("使用電阻率欄位設定起始模型電阻率...完成!")  
elif temp_json_data["Inversion_Setting05_startModel"] == -3 :    
    #--------------------------------------------
    # 建立網格
    temp_mesh = pg.Mesh(2)
    ...
    #--------------------------------------------
    write_log_append(f"全網格數量={mesh.cellCount()},研究區域網格數量={np.sum(mesh.cellMarkers() == 2)}.")
    write_log_append(f"起始模型網格數量={temp_mesh.cellCount()}.")
    if temp_mesh.cellCount() == np.sum(mesh.cellMarkers() == 2) :
        write_log_append("起始模型網格數量與研究區域網格數量相等!")
        write_log_append("使用電阻率欄位設定起始模型電阻率...")
        if temp_mesh.haveData("Resistivity"):
            write_log_append(f'設定起始模型電阻率(Min/Max) = {np.min(temp_mesh["Resistivity"])}/{np.max(temp_mesh["Resistivity"])}')
...
with open(Output_Inversion_Log, 'w', encoding='utf-8') as f:
    ...
    'Lambda_Index,Lambda,RMS,RRMS[%],Chi2,Phi,PhiD,PhiM\n'    
    f.write(line)
    #--
#write_log_append('儲存CSV檔案...完成!')
#--
write_log_append("用data建立ERTManager物件...")
mgr = ert.ERTManager(Ohm_data, sr=Inversion_SR)    
write_log_append("建立ERTManager物件...完成")
#--
write_log_append("使用管理器設定PostStep...")
# 告訴反演器：每次迭代結束，就去執行一次「Inversion_Info_Update」
mgr.inv.setPostStep(Inversion_Info_Update)
write_log_append("使用管理器設定PostStep...完成!")
#--
write_log_append("**使用管理器運行逆推...")
#--
if temp_json_data["Inversion_Setting00_getOptimizeLamda_Enable"] == 'Yes' :
    #--
    # 第0次(沒刪除資料)
    Current_Remove_Runs = 0
    write_log_append(f'逆推進度({Current_Remove_Runs}/{Inversion_Remove_Data_Times})...')
    Continue_Inversion_startModel = Inversion_startModel
    for lamda_index in range(1):
        Current_lamda_number =  lamda_index + 1
        write_log_append(f"使用自動lam進行逆推...")
        Inversion_lam = temp_json_data["Inversion_Setting00_getOptimizeLamda_StartingLamda"]
        Inversion_maxIter = 1
        if lamda_index == 0 :
            write_log_append("沒有可接關的模型，用參數決定的起始模型!")
            ...
            write_log_append("有可接關的模型，採用此模型作為起始模型!")
        ...
        write_log_append(f"使用自動lam進行逆推...完成!")
        write_log_append('更新一個「接關的起始模型」...')
        Continue_Inversion_startModel = mgr.inv.model
        write_log_append('更新一個「接關的起始模型」...完成')
        #--
        Output_Inversion_Log = os.path.join(temp_json_data['OutputMainFolderPath'],f'{Main_FileName}_03_Inversion_Log.csv')
        write_log_append('載入 Output_Inversion_Log 檔案...')
        try:
            #--------------------------------------------
            ...
            write_log_append(f"錯誤：找不到檔案 {Input_v299Scsv_FileName}")
            exit(1) # 回傳1表示一般性錯誤
        except Exception as e:
            write_log_append(f"程式運行時發生錯誤: {e}")
            exit(1) # 回傳1表示一般性錯誤
        #--
        ...
        write_log_append('載入 Output_Inversion_Log 檔案...完成!')
        #--
        # 目標函數下降比例: dPhi = current_Phi / last_Phi
        ...
        #write_log_append(f"程式結束運行...時間: {program_end_time.strftime('%Y-%m-%d %H:%M:%S')}")
        write_log_append(f"花費時間: {hours:02d}時 {minutes:02d}分 {seconds:02d}.{milliseconds:03d}秒")
        write_log_append('--')
        # 輸出花費時間檔案 
        Output_Inversion_Log = os.path.join(temp_json_data['OutputMainFolderPath'],f'{Main_FileName}_03_Inversion_Log.csv')+'.time'
        #write_log_append('儲存time檔案...')
        if lamda_index == 0 :
            file_mode = 'w'
        else :
            file_mode = 'a'
        with open(Output_Inversion_Log, file_mode, encoding='utf-8') as f:
            ...
            f.write(line+"\n")
            #--
        #write_log_append('儲存time檔案...完成!')
        #--
        #--------------------------------------------
    write_log_append(f'逆推進度({Current_Remove_Runs}/{Inversion_Remove_Data_Times})...完成!')
    #--
else:
    #--
    # 第0次(沒刪除資料)
    ...
    for lamda_index in range(Inversion_lam_list_count):
        Current_lamda_number =  lamda_index + 1
        write_log_append(f"使用第{Current_lamda_number}個lam進行逆推(lam={Inversion_lam_list[lamda_index]})...")
        ...
        if lamda_index == 0 :
            ...
            write_log_append("有可接關的模型，採用此模型作為起始模型!")
            mgr.invert(Ohm_data,mesh=mesh, lam=Inversion_lam, maxIter=Inversion_maxIter, startModel=Continue_Inversion_startModel, verbose=Inversion_verbose, limits=[Inversion_limitModelCellMinValue, Inversion_limitModelCellMaxValue])
        write_log_append(f"使用第{lamda_index+1}個lam進行逆推(lam={Inversion_lam_list[lamda_index]})...完成!")
        write_log_append('更新一個「接關的起始模型」...')
        Continue_Inversion_startModel = mgr.inv.model
        write_log_append('更新一個「接關的起始模型」...完成')
        #--
    ...
    for i in range(Inversion_Remove_Data_Times):
        ...
        # 執行刪除
        write_log_append(f"刪除之前的觀測資料數量: {Ohm_data_All_count}")
        Ohm_data.remove(temp_remove_mask)
        Ohm_data_Accept_count = Ohm_data.size()
        write_log_append(f"刪除之後的觀測資料數量: {Ohm_data_Accept_count}")    
        write_log_append(f'已刪除 {temp_Remove_Data_Count} 筆資料 (約 {Inversion_Remove_Data_Percentage}%)')
        #--
        # 2. 重新運行逆推
        #--
        write_log_append("用data建立ERTManager物件...")
        ...
        for lamda_index in range(Inversion_lam_list_count):
            ...
            with open(Output_Inversion_Log, file_mode, encoding='utf-8') as f:
                ...
                if (dPhi > (1.0 - mgr.inv.minDPhi / 100.0)) and mgr.inv.iter >= 2:
                    line=(f"Remove_Runs#{remove_runs_list[-1]}({remove_data_list[-1]}%),Iteration#{iteration_list[-1]}，運行結束原因: 已達到中止準則「dPhi({(1-dPhi)*100:.2f}%) < minDPhi({mgr.inv.minDPhi}%)」。")
                elif mgr.inv.chi2() <= 1 and mgr.inv.stopAtChi1:
                    line=(f"Remove_Runs#{remove_runs_list[-1]}({remove_data_list[-1]}%),Iteration#{iteration_list[-1]}，運行結束原因: 已達到中止準則「chi² <= 1」。目前 chi² = {mgr.inv.chi2():.2f}。")
                elif mgr.inv.iter == Inversion_maxIter:
                    line=(f"Remove_Runs#{remove_runs_list[-1]}({remove_data_list[-1]}%),Iteration#{iteration_list[-1]}，運行結束原因: 已達到設定的最大迭帶次數「iter == maxIter」。目前 iter = {mgr.inv.iter}。")   
                else: 
                    line=(f"Remove_Runs#{remove_runs_list[-1]}({remove_data_list[-1]}%),Iteration#{iteration_list[-1]}，運行結束原因: ???。目前 iter = {mgr.inv.iter}。")   
                write_log_append(line)
                f.write(line+"\n")
            #--
            #--------------------------------------------
    ...
    for Remove_Runs_ID, Remove_Runs_Color in zip(Unique_Remove_Runs_list, Unique_Remove_Runs_colors):
        ...
        # 原本可能會是[0,1,0,1]
        #ax.plot(Iteration_list[mask], RRMS_list[mask], color=Remove_Runs_Color, alpha=0.5, linewidth=1.2, label=f'Remove Runs #{Remove_Runs_ID}, IterCount={np.max(Iteration_list[mask])}')
        # 就要修改為[0,1,2,3]來繪圖
...
milliseconds = int((total_seconds % 1) * 1000)
write_log_append(f"程式結束運行...時間: {program_end_time.strftime('%Y-%m-%d %H:%M:%S')}")
write_log_append(f"最終花費時間: {hours:02d}時 {minutes:02d}分 {seconds:02d}.{milliseconds:03d}秒")
write_log_append('--')
# 輸出花費時間檔案 
Output_Inversion_Log = os.path.join(temp_json_data['OutputMainFolderPath'],f'{Main_FileName}_03_Inversion_Log.csv')+'.time'
#write_log_append('儲存time檔案...')
...
#--
# 移動Log檔案到輸出資料夾
Output_Log_FileName = os.path.join(temp_json_data['OutputMainFolderPath'],'ERTMaker_Inversion2D.log')
if os.path.dirname(Output_Log_FileName): # 防止空路徑報錯
    os.makedirs(os.path.dirname(Output_Log_FileName), exist_ok=True)
# 移動檔案
shutil.move('ERTMaker_Inversion2D.log', Output_Log_FileName)
#--------------------------------------------
```
