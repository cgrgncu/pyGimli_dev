# ERTMaker_CreateMesh_v20251030a.py
+ 作者: HsiupoYeh
+ 更新日期: 2025-10-30
...
\Lite_PyGimli_Env\Input_ERTMaker_CreateMesh\CreateMeshSettings.json
\Lite_PyGimli_Env\ERTMaker_CreateMesh_v20251030a.py
\Lite_PyGimli_Env\RUN_ERTMaker_CreateMesh_v20251030a.bat
```
+ 呼叫方式: 運行「RUN_ERTMaker_CreateMesh_v20251030a.bat」檔案，檔案內容:
```batch
.\PythonEnv\Python.exe ERTMaker_CreateMesh_v20251030a.py
PAUSE
```
...
```json
{
"CreateMeshSettings_Version":"v20251030a",
"CreateMeshSettings_Author":"HsiupoYeh",
...
"TempFile12_BasicMeshVTK_FileName_Readme":"不論Winodws或Linux或MAC作業系統一律使用「/」描述檔案路徑與資料夾路徑。",
"TempFile12_BasicMeshVTK_FileName":"Temp/CreateMesh_06_BasicMeshSetTopo.vtk",
...
"StudyAreaMesh_GridSurfaceNode_Readme":"逐個點在X方向上應為遞增的數字，不可填入null；在Z方向上填入對應高程，允許部分缺值(填入null)，將進行內插。",
"StudyAreaMesh_GridSurfaceNode_Readme2":"X[m],Z[m]",
...
#                PNG, VTK 和 JSON 檔案。
#                必須讀取指定位置「Input_ERTMaker_CreateMesh/CreateMeshSettings.json」的JOSN檔案。
#                設定檔(CreateMeshSettings.json)中有說明各參數意義。
#**************************************************************************
import json
import os
import numpy as np
...
#--------------------------------------------
print('--')
print('ERTMaker_CreateMesh運作開始!')
#--------------------------------------------
# 指定檔案名稱
json_file_name = 'Input_ERTMaker_CreateMesh/CreateMeshSettings.json'
#--------------------------------------------
# 讀取檔案
# 開啟並讀取 JSON 檔案
print("讀取 .json 檔...")
...
temp_filename=temp_json_data["TempFile12_BasicMeshVTK_FileName"]
os.makedirs(os.path.dirname(temp_filename), exist_ok=True)
if os.path.exists(temp_filename): # 檢查檔案是否存在
    os.remove(temp_filename)     # 如果存在，就刪除
#--
...
mesh['S_Coverage'] = np.ones(mesh.cellCount())
# 輸出
temp_output_filename=temp_json_data["TempFile12_BasicMeshVTK_FileName"]
mesh.exportVTK(temp_output_filename)
#-------------------------------------------- 
# 整理 MeshBCMarkers
#--    
