...
#endif
  ```
+ 重點就是沒有最佳化lam，根本沒實作，然後Lam_Factor只會白癡依照倍數每次疊代都換，這根本不合理不好用，只好用預設的倍數為1，等於不改動。
+ 然後求解的問題就是簡單的考慮最小化目標函數PhiD+lam*PhiM。
+ phiD=sum(殘差/Error)
+ phiM=與網格有關的平滑度。但如果要引入更多模型控制項，不太容易知道在幹嘛，所以都不設定=用預設。
+ 結果就是phiD phiM 都不太需要控制，只要管lam。然後lam會希望chi^2=1是最好的。
