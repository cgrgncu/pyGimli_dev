...
+ pyGIMLI的ERT
  ...
  + 順推計算: 採用有限元素法 (FEM) 來數值求解描述地下電流分佈和電位場的偏微分方程 (PDE) 。
    ...
    6. PyGimli的2D順推，預設採用2.5D模式(兼顧2D模型結構與3D電流分佈效應的計算方式)，此時軟體將啟用傅立葉轉換功能(使用傅立葉積分來處理3D電流分佈)。
    ...
    + 逆推問題的本質是利用有限的觀測資料 (Nd個數據) 去求解一個參數數量通常也有限的模型 (Nm個參數)。
      ...
      + 在ERT逆推中，核心挑戰在於在模型的複雜度（由網格細密程度決定的參數數量 Nm）與觀測數據提供的信息量（數據數量 Nd、質量及敏感度分佈）之間找到平衡。
        ...
        + 如果有很多的數據，想要降低數量，可以考慮選擇敏感度閾值篩選。
          + 靈敏度計算的實作是在core/src/bert/bertJacobian.cpp，採用伴隨法，而不是暴力法(1.05倍擾動)。計算需要網格幾何、ABMN、所有網格內的電阻率值。每次電阻率不同都要重新計算一次。
        ```python
